...
curdir = "".join(f"{el}\\" for el in __file__.split("\\")[:-1])
...
allh = cyrus + sterling + oakland + palmer
cyrush, sterlingh, oaklandh, palmerh = [], [], [], []

for el in range(15):
    house = input(f"House ({el+1}/15):").upper()
    try:
        house = int(house)
    except:
        pass
    while house not in allh or house in cyrush + sterlingh + oaklandh + palmerh:
        house = input(f"Invalid, please re-enter\nHouse ({el+1}/15):")
        try:
            house = int(house)
        except:
            pass
